...
std::vector<uint8_t> FastconController::single_control(uint32_t light_id_,
  ...
  //  - high 4 bits: (data_len + 1) masked to 4 bits (as per your existing intent)
  const uint8_t type_nibble = 0x02 & 0x0F;
  const uint8_t len_nibble  = static_cast<uint8_t>((data_len + 1) & 0x0F);
  result_data[0] = static_cast<uint8_t>((len_nibble << 4) | type_nibble);

  // Header byte 1: light_id (1 byte). Match previous behavior, but warn if truncated.
  if (light_id_ > 0xFF) {
